...
# Use user-provided path if it exists
if(NOT "${ONNXRUNTIME_ROOTDIR}" STREQUAL "${DEFAULT_ONNXRUNTIME_ROOT}" 
   AND EXISTS "${ONNXRUNTIME_ROOTDIR}/include/onnxruntime_cxx_api.h")
    ...
    # Copy to default folder to keep RPATH logic simple
...
# If default folder not found, download and extract
if(NOT EXISTS "${ONNXRUNTIME_ROOTDIR}/include/onnxruntime_cxx_api.h")
    ...
    # The extracted folder is versioned; rename to stable "onnxruntime"
    file(GLOB ONNX_EXTRACTED_DIR "${CMAKE_CURRENT_SOURCE_DIR}/onnxruntime-*")
    list(GET ONNX_EXTRACTED_DIR 0 ONNX_RUNTIME_DIR)
    execute_process(
        COMMAND ${CMAKE_COMMAND} -E rename "${ONNX_RUNTIME_DIR}" "${DEFAULT_ONNXRUNTIME_ROOT}"
    )
    set(ONNXRUNTIME_ROOTDIR "${DEFAULT_ONNXRUNTIME_ROOT}")
endif()
